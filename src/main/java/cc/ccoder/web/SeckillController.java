...
public class SeckillController {
    ...
    public String detail(@PathVariable("seckillId") Long seckillId, Model model) {
        if (seckillId == null) {
            //请求重定向到列表页
            return "redirect:/seckill/list";//为什么重定向
        }
        Seckill seckill = seckillService.getById(seckillId);
        if (seckill == null) {
            return "forward:/seckill/list";//为什么转发
        }
    ...
    public SeckillResult<SeckillExecution> execute(@PathVariable("seckillId") Long seckillId,
        ...
        //老师备课备的不好，这个excution方法里面的try-catch处理明显冗余，
        // catch块里面的返回值应该使用SeckillResult(boolean success , String error)这个构造比较合理
        //这里用两种SeckillResult的两种构造方法都可以，如果用含String error的可以使用枚举.getStatInfo()来获取异常信息
        // 用含SeckillExecution execution直接内含异常信息，你说备课不好是不是略显轻狂
        //Service 中的抛出异常是为了让 Spring 能够回滚，Controller 中捕获异常是为了将异常转换为对应的 Json 供前台使用，缺一不可
        try {
